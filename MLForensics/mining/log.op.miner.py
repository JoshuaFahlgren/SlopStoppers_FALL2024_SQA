    ...
    for root_, dirnames, filenames in os.walk(path2dir):
        for file_ in filenames:
            ...
            if os.path.exists(full_path_file):
                if file_.endswith(PY_FILE_EXTENSION) and (
                    checkIfParsablePython(full_path_file)
                ):
                    valid_list.append(full_path_file)
    valid_list = np.unique(valid_list)

    logger.info(
        f"{FILE_NAME} | getAllPythonFilesinRepo: {len(valid_list)} Python files find in {path2dir}"
    )
    ...
    if isinstance(node_, ast.Call):
        ...
        if isinstance(func_, ast.Attribute):
            ...
            if isinstance(func_parent, ast.Name):
                ...
                for x_ in range(len(funcKeys)):
                    ...
                    if isinstance(funcKey, ast.keyword):
                        call_arg_list.append(
                            (funcKey.arg, constants.INDEX_KW + str(x_ + 1 + index))
                        )
            ...
            if isinstance(func_parent, ast.Attribute):
                ...
                full_list.append(
                    (func_parent_name, func_name, funcLineNo, call_arg_list)
                )
            ...
            if isinstance(func_parent, ast.Call):
                ...
                if isinstance(func_parent_name, ast.Name):
                    full_list.append(
                        (func_parent_name.id, func_name, funcLineNo, call_arg_list)
                    )
    return full_list
...
def getLogStatements(pyFile):
    ...
    for func_decl_ in func_decl_list:
        func_parent_id, func_name, funcLineNo, call_arg_list = (
            func_decl_  # the class in which the method belongs, func_name, line no, arg_list
        )
