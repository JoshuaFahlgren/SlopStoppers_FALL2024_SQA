...
def dumpContentIntoFile(strP, fileP):
    logger.info(f"{FILE_NAME} | dumpContentIntoFile: Writing {strP} into {fileP}")
        ...
        for file_ in filenames:
            ...
            if os.path.exists(full_path_file):
                if (file_.endswith("py")) or (file_.endswith("ipynb")):
                    ...
                    pythonFileContent = [
                        z_.lower() for z_ in pythonFileContent if z_ != "\n"
                    ]
...
def getDevEmailForCommit(repo_path_param, hash_):
    ...
    author_emails = [
        x_.replace(hash_, "") for x_ in author_emails if x_ != "\n" and "@" in x_
    ]
    author_emails = [
        x_.replace("^", "") for x_ in author_emails if x_ != "\n" and "@" in x_
    ]
    author_emails = [
        x_.replace("!", "") for x_ in author_emails if x_ != "\n" and "@" in x_
    ]
...
def getDevDayCount(full_path_to_repo, branchName="master", explore=1000):
    ...
    all_day_list = [
        datetime(
            int(x_.split("-")[0]), int(x_.split("-")[1]), int(x_.split("-")[2]), 12, 30
        )
        for x_ in all_time_list
    ]
    try:
...
def cloneRepos(repo_list, dev_threshold=3, python_threshold=0.10, commit_threshold=25):
    ...
    for repo_batch in repo_list:
        for repo_ in repo_batch:
            ...
            print("Cloning ", repo_)
            dirName = (
                "../FSE2021_REPOS/" + repo_.split("/")[-2] + "@" + repo_.split("/")[-1]
            )  ## '/' at the end messes up the index
            cloneRepo(repo_, dirName)
            ### get all count
            checkPattern, dev_count, python_count, commit_count, age_months = (
                0,
                0,
                0,
                0,
...
if __name__ == "__main__":
    ...
    chunked_list = list(
        makeChunks(list_, 100)
    )  ### list of lists, at each batch download 1000 repos
